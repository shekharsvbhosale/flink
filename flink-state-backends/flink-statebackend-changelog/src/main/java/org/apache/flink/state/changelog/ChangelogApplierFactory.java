...
interface ChangelogApplierFactory {
    ...
    <K, N, UK, UV> StateChangeLogReader.ChangeApplier<K, N> forMap(
            InternalMapState<K, N, UK, UV> map, InternalKeyContext<K> keyContext);

    <K, N, T> StateChangeLogReader.ChangeApplier<K, N> forValue(
            InternalValueState<K, N, T> value, InternalKeyContext<K> keyContext);

    <K, N, T> StateChangeLogReader.ChangeApplier<K, N> forList(
            InternalListState<K, N, T> list, InternalKeyContext<K> keyContext);

    <K, N, T> StateChangeLogReader.ChangeApplier<K, N> forReducing(
            InternalReducingState<K, N, T> reducing, InternalKeyContext<K> keyContext);

    <K, N, IN, SV, OUT> StateChangeLogReader.ChangeApplier<K, N> forAggregating(
